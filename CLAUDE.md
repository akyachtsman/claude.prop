...
| S16 | Change flash | After an edit, output values that changed get a soft fading `.flash` overlay; none on initial load or on edits with no computed effect | Flash on load, no flash on a rippling edit, or flash on an inert edit |
