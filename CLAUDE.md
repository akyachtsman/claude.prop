...
| S12b | Estimated expense seed | Each estimated expense is filled/refreshed from its own driver, **per-key**: **tax** = `offer × 0.012` (on offer/Asking edit only); **insurance** = `rentableSF × $/SF` keyed to the **Property Type** dropdown (`INSURANCE_RATE` in `dashboard.js`: Office .65, Retail 1.00, Industrial .70, Warehouse .30, Multifamily/Residential 1.00, Commercial .80 default) — on SF/type edit only, so an SF edit never disturbs a goal-sought offer's taxes. A blank estimate fills, and a value **we** generated (`seeded`) **recomputes** when its driver moves; a fixture's real figure or a typed actual (`seeded!==true`) is never touched, regardless of `estimated` or a returning cloud copy, and goal-seek never seeds. So S5 fixture is unchanged | Tax/insurance don't seed on their own driver, an SF edit seeds taxes, a type change doesn't recompute a still-auto insurance, a typed/fixture value gets overwritten, or the type rate is ignored |
