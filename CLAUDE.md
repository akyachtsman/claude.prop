...
| S15 | Target CAP/DSCR goal-seek | The editable **Target CAP/DSCR** band cells are a **goal-seek**: typing a value back-solves the Offer Price (`NOI÷cap` / `PV(loan)÷LTV`) so the **actual** CAP/DSCR becomes that number — Target moves the offer, not the pills. They start **empty** (a value equal to the benchmark renders blank so a legacy 8%/1.25 default never reads as a set target). The verdict pills check a **fixed hard-coded benchmark** (`BENCHMARK_CAP` 8% / `BENCHMARK_DSCR` 1.25 in `model.js`), independent of the Target | Target pre-filled on load, typing a Target doesn't move the offer / actual CAP, or a pill checks the Target instead of the fixed 8%/1.25 |
