...
| S12b | Estimated expense seed | On a **new** deal, setting the offer (direct or Asking→Offer) seeds the **estimated** tax & insurance to `offer × 0.012`; a manual edit overwrites it (drops `estimated`, never re-seeded); goal-seek moves the offer without re-seeding (stays exact). Sample/demos carry real actuals (`estimated:false`) and are never seeded — S5 fixture unchanged | Tax/insurance don't seed on a new deal, an overwritten value re-seeds, goal-seek re-seeds, or the sample's figures change |
