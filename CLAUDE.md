...
| S38 | Target sweeps | Target CAP and Target DSCR share one combined, double-wide deal-cell (`.deal-cell--target-combo`), stacked one row above the other (`.target-row`) instead of two separate side-by-side cells — freeing the full width for a slider. Each row independently keeps its typed **goal-seek** mode (default, unchanged — permanently moves the offer, S15) and gains a **sweep** mode, flipped by its own `.cap-mode-btn`. **Cap sweep**: fixed **5%–20%** range, 0.05% steps, live-previews the ENTIRE dashboard as if Offer Price were `NOI ÷ swept-cap`, holding each loan's **dollar** amount fixed (so NOI/DSCR/loan amount/payment never move — only the valuation/equity side does: CAP, All-In Cost, Cash-on-Cash, WACC, IRR, NPV, Price/SF, the topbar pills). **DSCR sweep**: fixed **1–5** range, 0.01 steps, reuses the same PV(loan)÷LTV solve as the DSCR goal-seek — the loan amount floats naturally with it (DSCR is fundamentally about debt sizing, so nothing is held fixed here). The two sweeps are **mutually exclusive** — turning one on turns the other off, since both ultimately drive the same simulated Offer Price. Whichever is active anchors its slider to the deal's own current CAP/DSCR (clamped into range) on entry, so nothing visibly jumps. Offer Price renders burgundy (`.input--simulated`, `--color-simulated`) while either is live. Dragging never calls `onEdit`/`onCommit` — no undo step, no auto-save, no S16 flash markers (suppressed during the drag) — and a reload always shows the real deal untouched, same transient spirit as the goal-seek fields themselves. Both disabled whenever the dashboard is locked (forces any active sweep off) | Target CAP/DSCR aren't stacked in one combined cell, either sweep's range/step is off, both sweeps can be active simultaneously, sweeping moves/saves the real `prop.offer.offerPrice` or `prop.loans`, NOI/DSCR move during a cap sweep (loan not held fixed) or the loan amount stays frozen during a DSCR sweep, sweeping survives a reload, the offer isn't burgundy while simulated, or either switch/slider stays usable while the dashboard is locked |
