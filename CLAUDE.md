...
| S31 | URL import | The list's primary **Import from URL** action opens a modal; pasting a Crexi listing URL calls the **`import-listing` Edge Function** (`js/supabase.js` `importListing`), which returns a normalized property the app saves and opens. An unsupported/invalid URL shows an inline error and doesn't navigate; a valid one lands the deal (Subtype, Source, photos, …). `+ New property` (blank) and `Load sample deal` remain. The function endpoint is stubbed in `tests/_supabase-mock.js` (Crexi URL → canned property, else 400), so the flow runs without network | No Import-from-URL button/modal, a valid URL doesn't create+open a property, a bad URL navigates or shows no error, or imported fields (Subtype/Source/photos) are missing |
