...
| S36 | Compare picker | The Compare picker (`js/views/compare.js`) is a checkbox in front of every property row, not a chip list — checked properties are included in the table/side-by-side comparison and sortable; unchecking a property drops it out of the comparison and sinks its row below a **"Not included in comparison"** divider (`.compare-divider`), dimmed (`.compare-row--off`); re-checking restores it to the included group and the divider disappears once nothing is excluded. No minimum-selection floor — any property can be unchecked, including down to the last one | No checkbox per row (still a chip list), unchecking doesn't remove the property from the table, no divider/dimming for excluded rows, or a checkbox can't be re-checked back in |
