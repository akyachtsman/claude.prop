...
| S12b | Estimated expense seed | Setting the offer (direct or Asking→Offer) fills a **blank** (`$0`) estimated tax & insurance **once** to `offer × 0.012` (workbook default). The `amount===0` guard means it only ever fills an unset field — a fixture's real figure or a typed actual is never overwritten (regardless of `estimated` flag or a returning cloud copy), and goal-seek never re-seeds (stays exact). So S5 fixture is unchanged | A blank new deal's tax/insurance don't seed, a filled value gets re-seeded/overwritten, or a fixture's figures change |
