...
| S12b | Estimated expense seed | A **blank** (`$0`) estimated expense fills **once** from its driver: **tax** = `offer × 0.012` (on offer/Asking set); **insurance** = `rentableSF × $/SF` where the rate is keyed to the **Property Type** dropdown (`INSURANCE_RATE` in `dashboard.js`: Office .65, Retail 1.00, Industrial .70, Warehouse .30, Multifamily/Residential 1.00, Commercial .80 default). The `amount===0` guard means it only ever fills an unset field — a fixture's real figure or a typed actual is never overwritten (regardless of `estimated` flag or a returning cloud copy), and goal-seek never re-seeds. So S5 fixture is unchanged | A blank new deal's tax/insurance don't seed on their drivers, a filled value gets re-seeded/overwritten, the type rate is ignored, or a fixture's figures change |
