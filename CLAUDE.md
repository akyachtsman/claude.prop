...
| S34 | Offer Price lock | A **`.lock-btn`** (🔓/🔒) sits in front of the Offer Price field in the deal-summary band, toggling `prop.offer.locked` (persisted, auto-saved, undoable). Unlocked = plain white, editable input. Locked = shaded (`.input--locked`, `--color-bg`, distinct from the accent-tinted "estimate" fill) and `readOnly`, so it resists direct typing — **and** the two other paths that can move it: the Target CAP/DSCR goal-seek and the Asking→Offer sync both no-op while locked. Defaults unlocked (`false`), so S5/S12/S15 hold unchanged | No lock button, locking doesn't shade/read-only the field, a locked offer still moves via goal-seek or an Asking Price edit, or the locked state doesn't survive reload |
