...
| S15 | CAP/DSCR goal-seek (split from Target) | Two separate concerns: the editable **Target CAP/DSCR** band cells start **empty** (an optional per-deal override — a pre-filled value would read as a target the user set) while the verdict pills fall back to a **hard-coded benchmark** (`BENCHMARK_CAP` 8% / `BENCHMARK_DSCR` 1.25 in `model.js`; a set Target > 0 overrides it), and the **blank** "Solve offer for CAP/DSCR" inputs under Offer Price back-solve the offer (`NOI÷cap` / `PV(loan)÷LTV`) then clear — one-shot, never touching the Target or persisting | Target pre-filled on load, pills lose their verdict when Target empty, a set Target doesn't override the benchmark, a solve input pre-filled, or a solve doesn't move the offer |
