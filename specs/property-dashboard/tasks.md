...
- [x] **T1** `js/format.js` — money `$#,##0`, money-cents, percent (whole + 2dp),
  ratio (2dp), K/M suffix, "Jun 4" date, and `fmtOrDash(v)` → "—" for
  null/NaN/undefined. Pure functions. (FR-23, FR-12)
- [x] **T2** `js/model.js` — pure `compute(property)` → metrics: loan payments
...
- [x] **T3 [P]** `js/sample.js` — the 1042 Maple Ave Property object + a frozen
  `EXPECTED` metrics fixture (known-good values) for the fidelity test. (SC-1)

## B. Persistence
- [x] **T4** `js/store.js` — repository over localStorage key `propanalytics.v1`:
...
## D. Views
- [x] **T7** `js/notes.js` — the workbook methodology text (rows 20–29) verbatim
  as data for the panel. (FR-22)
- [x] **T8** `js/views/dashboard.js` — render the one-screen dashboard bound to a
...
- [x] **T13** S5 calc fidelity — load sample, assert all 12 KPIs == EXPECTED. (SC-1)
- [x] **T14 [P]** S6 live recalc — change offer, assert CAP updates, no button. (SC-2)
- [x] **T15 [P]** S7 persistence — save, reload, property present. (SC-3)
- [x] **T16 [P]** S8 compare — 2 properties, best/worst + verdict badges. (SC-4)
- [x] **T17 [P]** S9 empty/zero — zeroed property renders "—", no NaN. (SC-7)
- [x] **T18 [P]** S10 export/import round-trip — identical set restored. (SC-6)
- [x] **T19 [P]** S11 one-screen — 1440×900 no vertical scroll, all data present. (SC-5)
- [x] **T20** generic gates in the suite: console-error, delete-confirm dismiss
  (button+Esc+backdrop), controls-clicked, ENTRY load. (test.md gates)
- [x] **T21** `CLAUDE.md` — fill UI Test Configuration + Project-Specific Test
