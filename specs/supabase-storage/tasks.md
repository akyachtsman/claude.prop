...
- [x] **T5. [P]** Vendor `@supabase/supabase-js` v2 → `js/vendor/supabase-js.js`
...
- [x] **T6. [P]** `js/config.js` — export `SUPABASE_URL`
...
- [x] **T7.** `js/supabase.js` — `createClient(url, key, { auth: { flowType: 'pkce',
...
- [x] **T8.** Extract the current localStorage code in `js/store.js` into an internal
  ...
  **No behavior change.** Verify: full logged-out S5–S22 + app suite still green.
  Depends: —
- [x] **T9.** Add `cloudBackend` to `store.js`: cache on `propanalytics.cloud.<uid>`;
...
- [x] **T10.** Offline choke-point: in `store.save`/`store.remove`, when
  ...
  **reject** without mutating cache and `toast("You're offline — reconnect to edit.")`.
  Depends: T9.
- [x] **T11.** `store.setSession(session|null)` swaps `local`↔`cloud`; guard
  `refreshBuiltinSample()`/`seedDemos()` (`app.js` boot) to run on the **local**
  backend only. Depends: T8, T9.
- [x] **T12. Unit (Node) [P]:** store backend swap flips namespace; logged-out never
...
- [x] **T16.** Initial cloud fetch on sign-in: `select *` (RLS-scoped) → populate the
  **cloud** cache (never `propanalytics.v1`) → "Syncing…" state → re-render; on fetch
  error → cached read-only + toast. Depends: T9, T14.
- [x] **T17.** First-sign-in reconcile (D6, fixed order): if local deals exist → the
...
- [x] **T18.** 401-vs-blip on write-through: auth/401 error → route to `SIGNED_OUT`
  handling (local backend + "signed out — sign in again"); transient error → "will
  re-sync" toast + cache holds. Depends: T9, T14.
- [x] **T19.** Offline read-only UI: banner + disable dashboard inputs / +New / Load
...
## Phase E — Verify + ship
- [x] **T20. Unit (Node) [P]:** D6 upload/dedup — upload upserts, gap-seed fills only
...
- [x] **T21. DB integration (MCP) [P]:** an authenticated-role `upsert` produces the
  row; a second upsert by same `(user_id,id)` updates (no duplicate). Depends: T4.
- [x] **T22. Playwright:** auth UI states (signed-out / check-email / signed-in /
...
- [x] **T23.** CLAUDE.md: add S23+ scenario rows (auth states, cloud persist, offline
  read-only, upload-on-sign-in) + UI-test config notes. Depends: T22.
- [x] **T24.** Grep built/served assets for any `service_role`/secret key → must be
