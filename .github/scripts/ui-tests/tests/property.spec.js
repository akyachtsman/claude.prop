...
test('Estimated expense defaults — tax = offer×0.012, insurance = SF × property-type rate; seed once, overwritable', async ({ page }) => {
  await page.goto('./', { waitUntil: 'load' });
  await page.click('button:has-text("Add your first property")');
  await page.waitForSelector('.kpi-strip');
  // Insurance scales with the building: pick a Property Type (Warehouse → $0.30/SF),
  // then enter rentable SF and it fills once.
  await page.selectOption('select[aria-label="Type"]', 'Warehouse');
  await setField(page, 'input[aria-label="Rentable SF"]', '10000');
  await expect(page.locator('input[aria-label="Insurance amount"]')).toHaveValue('3000');   // 10000 × 0.30
  // Tax scales with price: setting the offer (via Asking) fills it once at offer × 0.012.
  ...
  // The seed only ever fills a BLANK field. Overwrite insurance, then change SF —
  // the filled value never re-seeds (a typed actual is never overwritten).
  await setField(page, 'input[aria-label="Insurance amount"]', '5000');
  await setField(page, 'input[aria-label="Rentable SF"]', '20000');
  await expect(page.locator('input[aria-label="Insurance amount"]')).toHaveValue('5000');
});
