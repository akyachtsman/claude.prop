...
test('Estimated expense defaults — tax & insurance seed from offer × 0.012 on a new deal, overwritable', async ({ page }) => {
  await page.goto('./', { waitUntil: 'load' });
  await page.click('button:has-text("Add your first property")');
  await page.waitForSelector('.kpi-strip');
  // Setting the offer (here via Asking → Offer) seeds the estimated tax & insurance
  // to offer × 0.012 — the workbook default for a fresh deal.
  ...
  // A manual entry overwrites the estimate; re-setting the offer then leaves the
  // overwritten tax alone while the still-estimated insurance keeps tracking.
  ...
  await expect(page.locator('input[aria-label="Insurance amount"]')).toHaveValue('24000');
});
